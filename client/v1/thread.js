...
import { ThreadEmptyError } from "./exceptions"
...
export default class Thread extends Resource {
   ...
   parseParams(json) {
      ...
      hash.inviter = new Account(this.session, json.inviter)
      this.items = _.map(json.items, item => {
         return new ThreadItem(this.session, item)
      })
      this.accounts = _.map(json.users, user => {
         return new Account(this.session, user)
      })
      this.leftUsers = _.map(json.left_users, user => {
         return new Account(this.session, user)
      })
      return hash
   }
   ...
   seen() {
      ...
      if (!firstItem) throw new ThreadEmptyError()
